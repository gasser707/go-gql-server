...
func (l *ImageLoader) Load(key int) ([]*custom.Image, error) {
...
func (l *ImageLoader) LoadThunk(key int) func() ([]*custom.Image, error) {
	l.mu.Lock()
...
func (l *ImageLoader) LoadAll(keys []int) ([][]*custom.Image, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ImageLoader) LoadAllThunk(keys []int) func() ([][]*custom.Image, []error) {
	results := make([]func() ([]*custom.Image, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ImageLoader) Prime(key int, value []*custom.Image) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*custom.Image, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *ImageLoader) unsafeSet(key int, value []*custom.Image) {
	if l.cache == nil {
